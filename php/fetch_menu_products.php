...
$query = ($queryParams==null) ? "SELECT p.id,p.name,p.image_path,p.price,menu.name as 'menuName',color.name as 'colorName',type.name as 'typeName',category.name as 'categoryName' FROM product p INNER JOIN menu ON menu.id=p.menuId
				INNER JOIN color ON color.id=p.colorId
				INNER JOIN category ON category.id=p.categoryId
				INNER JOIN type ON type.id = p.typeId WHERE p.menuId='$menuId' ORDER BY price ASC" : createQuery($queryParams,$menuId);
...
function createQuery($params,$mId)
	{
		$query="SELECT p.id,p.name,p.image_path,p.price,menu.name as 'menuName',color.name as 'colorName',type.name as 'typeName',category.name as 'categoryName' FROM product p INNER JOIN menu ON menu.id=p.menuId
				INNER JOIN color ON color.id=p.colorId
				INNER JOIN category ON category.id=p.categoryId
